  ...
  switch (target) {
    ...
    default:
      return InvalidArgument(
          "Unknown DyamicParameterBinding target enum value #%d", target);
  }
}
...
mlir::ArrayAttr ConvertDynamicParameterBindings(
  ...
  (void)dpb.ForEachBinding(
      [&](const DynamicParameterBinding::DynamicParameter& source,
        ...
        auto dynamic_parameter_target =
            ConvertDynamicParameterTarget(target.target);
        if (!dynamic_parameter_target.ok()) {
          assert(false && "Unknown hlo DynamicParameterBinding target");
          return dynamic_parameter_target.status();
        }
