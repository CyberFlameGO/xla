...
class SparseCustomCallToPackRewriter
  ...
  LogicalResult matchAndRewrite(mhlo::CustomCallOp op,
    ...
    const StringRef sparse_tranpose_call_name = "sparse_tensor_tranpose";
    ...
    if (op.getCallTargetName().equals(sparse_pack_call_name)) {
      ...
      return success();
    } else if (op.getCallTargetName().equals(sparse_tranpose_call_name)) {
      assert(op.getInputs().size() >= 2 && "Need argument and permutation");
      assert(op.getResults().size() == 1 &&
             "Must be transposing into one tensor");
      // Rebuild the permutation from the parameteers.
      unsigned sz = op.getInputs().size() - 1;
      llvm::SmallVector<int64_t> permutation_array(sz);
      for (int64_t i = 0; i < sz; i++) {
        auto constOp = op.getInputs()[i + 1].getDefiningOp<mhlo::ConstantOp>();
