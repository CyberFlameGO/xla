...
def get_var(environ_cp,
  ...
  """
  if not question:
    question = 'Do you wish to build XLA with {} support?'.format(
        query_item)
  if not yes_reply:
    yes_reply = '{} support will be enabled for XLA.'.format(query_item)
  if not no_reply:
...
def main():
  ...
  if is_windows():
    print('\nWARNING: Cannot build with CUDA support on Windows.\n'
          ...
          'For using XLA GPU on Windows, you will need to build/install '
          'XLA in WSL2.\n')
  ...
  # Disable NCCL if XLA is configured for CPU
  if gpu_platform_count == 0:
    write_to_bazelrc('build --config=nonccl')
